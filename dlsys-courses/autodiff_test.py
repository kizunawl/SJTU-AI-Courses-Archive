...
def test_reduce_sum():
    x1 = ad.Variable(name = "x1")
    y = ad.reduce_sum_op(x1)

    grad_y, = ad.gradients(y, [x1])
    x1_val = np.array([[1,2], [3,4], [5,6]])
    executor = ad.Executor([y, grad_y])
    y_val, grad_y_val, = executor.run(feed_dict = {x1: x1_val})

    expected_y_val = np.sum(x1_val, axis=0)
    expected_grad_y_val = expected_y_val + np.zeros_like(x1_val)
    assert np.array_equal(y_val, expected_y_val)
    assert np.array_equal(grad_y_val, expected_grad_y_val)