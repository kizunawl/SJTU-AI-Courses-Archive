...
class DivOp(Op):
    ...
    def gradient(self, node, output_grad):
        """TODO: Your code here"""
        return [1.0 / node.inputs[1],
                -1.0 * (node.inputs[0] / (node.inputs[1] * node.inputs[1]))]
...
class DivConstByOp(Op):
    ...
    def gradient(self, node, output_grad):
        """TODO: Your code here"""
        return [-1.0 * output_grad * node.const_attr / (node.inputs[0] * node.inputs[0])]
...
class LogOp(Op):
    ...
    def gradient(self, node, output_grad):
        """TODO: Your code here"""
        return [1.0 * output_grad / node.inputs[0]]
...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        if (node.matmul_attr_trans_A==False and node.matmul_attr_trans_B==False):
            assert len(input_vals[0][0]) == len(input_vals[1])
            return np.matmul(input_vals[0], input_vals[1])
        if (node.matmul_attr_trans_A==False and node.matmul_attr_trans_B==True):
            assert len(input_vals[0][0]) == len(input_vals[1][0])
            return np.matmul(input_vals[0], np.transpose(input_vals[1]))
        if (node.matmul_attr_trans_A==True and node.matmul_attr_trans_B==False):
            assert len(input_vals[0]) == len(input_vals[1])
            return np.matmul(np.transpose(input_vals[0]), input_vals[1])
        if (node.matmul_attr_trans_A==True and node.matmul_attr_trans_B==True):
            assert len(input_vals[0]) == len(input_vals[1][0])
